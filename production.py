...
email_password = 'kdpo givc nwoi mnhl'  # App-specific password kdpo givc nwoi mnhl
email_receivers = ['kshetty200319@gmail.com', 'kshetty2003@gmail.com', 'krrish.shetty@ggsf.edu.in']  # List of receiver emails
...
class DesktopApp(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('ABB Production Automata')
        ...
        self.label_sale_number = QLabel('Enter Sale Number:')
        layout.addWidget(self.label_sale_number)
        self.sale_number_input = QLineEdit()
        layout.addWidget(self.sale_number_input)
        
        self.label_customer_name = QLabel('Enter Customer Name:')
        layout.addWidget(self.label_customer_name)
        self.customer_name_input = QLineEdit()
        layout.addWidget(self.customer_name_input)
        
        self.label_sub_order = QLabel('Enter Sub Order Number:')
        layout.addWidget(self.label_sub_order)
        self.sub_order_input = QLineEdit()
        layout.addWidget(self.sub_order_input)
        
        self.label_line_item = QLabel('Enter Line Item Number:')
        layout.addWidget(self.label_line_item)
        self.line_item_input = QLineEdit()
        layout.addWidget(self.line_item_input)
    ...
    def send_email(self):
        ...
        sale_number = self.sale_number_input.text()
        customer_name = self.customer_name_input.text()
        sub_order = self.sub_order_input.text()
        line_item = self.line_item_input.text()

        msg = MIMEMultipart()
        msg['From'] = email_sender
        msg['To'] = ', '.join(email_receivers)  # Join multiple email addresses with a comma
        msg['Subject'] = f'Captured files of Production for Sales Order #{sales_order}'
        ...
        body = (
            ...
            f"Sub Order Number: {sub_order}\n"
            f"Line Item Number: {line_item}\n\n"
            f"Thank you.\n\n"
            f"Regards,\n"
            f"Automation Team"
        )
